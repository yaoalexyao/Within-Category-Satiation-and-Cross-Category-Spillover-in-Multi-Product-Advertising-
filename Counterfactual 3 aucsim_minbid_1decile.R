########################################################################################
# This file aims to run Counterfactual 3: predict the ctr, cpc, advertiser's revenue, ad host's revenue when minimum bid changes
...
########################################################################################
# Import data
datam=read.csv("Delivery_Match_14day.csv", header=T)
...
# load bid eqm
load("bideqm_14day.RData")

# replace bid by the bid eqm
datab$bid = interout[, 2]

########################################################################################
# check out the distribution of adrank
datab$adrank=datab$bid*datab$qs_post


# what if we look at the distribution of highest adrank per date_subcat_id
maxadrankpergroup = datab %>% group_by(date_subcat_id) %>% summarise(maxadrank = max(adrank))
minadrankvec_v2 = quantile(maxadrankpergroup$maxadrank, prob=(seq(.1, .9, .1)))
minadrank = minadrankvec_v2[1]

# we need to update adid2 in datab
databnew = datab %>% filter(adrank > minadrank)

summary(datab)
summary(databnew)


# Next we modify N_ad in datam
N_ad_temp = datam$N_ad

i=1

while(i<=length(unique(datam$date_subcat_id))){
  # we only update N_ad
  index1=which(datam$date_subcat_id==i)
  index2=which(datab$date_subcat_id==i)
  if(length(index2)>0){
    # modify
    N_ad_temp[index1]=sum(datab$adrank[index2]>minadrank)
  }
  i=i+1
  if(i%%100==0){
    cat(i, fill=TRUE)
...
summary(datam$N_ad)
summary(N_ad_temp)
datam$N_ad = N_ad_temp

# we then update N_pot_subcat
summary(datam$N_pot_subcat)

i=1

while(i<=nrow(datam)){
  n_temp = datam$N_pot_subcat[i]
  datam$N_pot_subcat[i:(i+n_temp-1)] = sum(datam$N_ad[i:(i+n_temp-1)]>0)
  i=i+n_temp
}

summary(datam$N_pot_subcat)
length(which(datam$N_pot_subcat==0))
...
while(i<=N_datecat_all){
  index=which(databnew$date_subcat_id==i)
  if(length(index)==0){
    subcat_id=datam$subcat_id3[match(i, datam$date_subcat_id)]
    # find all date_subcat_id of this subcat_id in databnew
    temp=unique(databnew$date_subcat_id[which(databnew$subcat_id3==subcat_id)])
    ...
    if(length(temp)>0){
      ...
      datecatmat[i, 3]=length(which(databnew$date_subcat_id==datecatmat[i, 2]))
    }
  }
  i=i+1
  if(i%%100==0){
...
while(i<=length(index)){
  ...
  if(length(temp)>0){
    ...
    datam$N_ad[j]=length(which(databnew$date_subcat_id==datecatidnew))
  }
  i=i+1
  if(i%%1000==0){
    cat(i, fill=TRUE)
...
# we then drop observations with N_ad == 0 and then update N_pot_subcat
datam = datam %>% filter(N_ad>0)

i=1
while(i<=length(unique(datam$pv_id2))){
  index=which(datam$pv_id2==i)
  datam$N_pot_subcat[index]=length(index)
  i=i+1
  if(i%%1000==0){
    cat(i, fill=TRUE)
  }
}
summary(datam %>% select(N_pot_subcat, N_ad))



########################################################################################

databnew=rbind(databnew, 0)
databnew$adid2[nrow(databnew)]=max(databnew$adid2[-nrow(databnew)])+1
databnew$rprice[nrow(databnew)]=median(databnew$rprice[-nrow(databnew)])
databnew$qs_post[nrow(databnew)]=median(databnew$qs_post[-nrow(databnew)])
databnew$vpc[nrow(databnew)]=median(databnew$vpc[-nrow(databnew)])
databnew$bid[nrow(databnew)]=median(databnew$bid[-nrow(databnew)])
...
# Add minadrank to the function
EPROFIT=function(n_ad_disp, n_pot_ad, ad_index, ctrvec,
                 b, QS, v, tao, u, minadrank){
  # Arguments
  ...
  # minadrank: minimum adrank set by the platform, which affects the lowest CPC
  ...
  while(i<=N){
    if(n_ad_disp[i]>0){
      ...
      # attach a 0 at the end of wbtemp for the calculation of cpc in case all 8 ads were chosen from one subcat
      wb_temp=c(wb_temp, 0)
      ...
      # update the last CPC to reflect the impact of minadrank
      cpc[(l+ntemp-1)] = max(minadrank/(QS_temp[out$ix[ntemp]]*error_temp[out$ix[ntemp]]), cpc[(l+ntemp-1)])
...
N_ad_all=length(unique(databnew$adid2))
N_datecat_all=max(unique(databnew$date_subcat_id))
...
ad_id_b=databnew$adid2
date_cat_id_b=databnew$date_subcat_id
QS_post=databnew$qs_post
vpc=databnew$vpc
bid=databnew$bid
summary(vpc)
summary(bid)
...
# ad_index: a M by 1 vector recording the rows in datab for all potential ads asscoiated with all potential subcat
...
while(i<=N_pv){
  ...
  while(j<=length(date_subcat)){
    ...
    if(length(indextemp)==0 | date_subcat[j]==0){
      # this means we assign ads to the hypothetical bidder in the last row of datab 
      # if a subcat has never appeared in the matching process in observed data
      count=count+1
      ad_index[k:(k+n_ad_vec[j]-1)]=nrow(databnew)
    }else{
...
while(m<=ndraw){
  ...
  if(m %% 1==0){
    ...
    output_1decile=list(profit=profitmat, pay=paymat, cpc=cpcmat, click=clickmat, vpc=vpcmat, n_subcat_disp=n_subcat_disp_mat, avg_n_ad_disp=avg_n_ad_disp_mat)
      save(output_1decile,
         file = "results_from_CF_aucsim_1decile.RData")
