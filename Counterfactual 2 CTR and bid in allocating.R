########################################################################################
# This file does the third counterfactual by incorporating both bids and CTR in the allocation process
...
#####This function calculates the ad click probability
...
choiceprobpred = function(database) {
    ...
    if (!is.null(apollo_inputs[[paste0(mdcev_settings$componentName,
      ...
      if (mdcev_settings$hasOutside)
        mdcev_settings$probs_MDCEV <- function(inputs) {
          ...
          #inputs$gamma <- mapply(function(g, a) apollo_setRows(g,
          #                                                     !a, 0), inputs$gamma, inputs$avail, SIMPLIFY = FALSE)
  ...
  apollo_prediction = function (model, apollo_probabilities = apollo_probabilities1, apollo_inputs, prediction_settings = list(), modelComponent = NA) {
    ...
    predictionamount = predictions$model[,1:9]  ####This is the # of clicks if needed
...
load("bideqm_14day_20231231.RData")
...
while(i<=N_pv){
  ...
  user_id=datam$user_id3[j] #user id: 1
  
  n_ad_vec=ceiling(datam$N_ad[pv_id_temp]) # 
  date_subcat=date_cat_id_s[pv_id_temp]  #
  ad_index=rep(0, sum(n_ad_vec))  #
  bid_temp =  rep(0, sum(n_ad_vec))
  bid_mean = rep(0, length(n_ad_vec))
  j=1
  k=1
  while(j<=length(date_subcat)){
    indextemp=which((date_cat_id_b %in% date_subcat[j])==TRUE)  #check which date_cat_id_b (from datab) is in data_subcat
    if(length(indextemp)==0 | date_subcat[j]==0){
      ...
      if(length(indextemp)!=n_ad_vec[j]){
        ...
        ad_index[k:(k+n_ad_vec[j]-1)]=indextemp  #replace the ad_index with the indextemp
        
        bid_temp00 = bid[indextemp]
        bid_temp00[bid_temp00>16] = 16
        
        bid_temp[k:(k+n_ad_vec[j]-1)] = bid[indextemp]
        if (n_ad_vec[j]>=3) {
          bid_mean[j] = mean( sort(bid_temp[k:(k+n_ad_vec[j]-1)] , decreasing = TRUE)[1:3] )
        } else {
          bid_mean[j] = mean( bid_temp[k:(k+n_ad_vec[j]-1)] )
  ...
  pv_list[[i]]=list(n_pot_subcat=n_pot_subcat, #18
                    pv_id=pv_id_temp, #1:18 (page view)
                    user_id=user_id,  #1
                    date_subcat=date_subcat, #date_subcat from datam
                    n_ad_vec=n_ad_vec, ad_index=ad_index,  #ad_index from datab
                    bid = bid_temp  ,
                    bid_mean = bid_mean)
...
while(m<=ndraw){
  ...
  while(k<=nrow(datam)){
    ...
    bid_mean = pv_list[[datam$pv_id2[k]]]$bid_mean
    bid_mean[bid_mean<=0] = 0.0001
    
    prob=exp(S_temp*bid_mean)/sum(exp(S_temp*bid_mean))
    n_ad_temp=rmultinom(1, 8, prob)
    n_ad_vec[k:(k+n_pot-1)]=n_ad_temp
    
    n_ad_temp2= n_ad_temp
    n_ad_temp2[n_ad_temp>0] =  1
    
  if (sum(n_ad_temp2[n_ad_temp>0])<8) {  #If there are 8 subcat already, it is fine.
      
    n_ad_disp = n_ad_temp
    
    pv_id_temp = datam$pv_id2[k]
    
    database1 = database0[database0$pv_id2== pv_id_temp, ]
    # change clicknew1-clicknew7 to 0
    database1[, 6:13] = 0
    database1[, 103:110] = 0
    
    # update other variables in database1, index_temp is index where datam$pv_id == i
    index_temp=pv_list[[pv_id_temp]]$pv_id  #here i is from 1 to 33k
    index_temp20 = which(n_ad_disp>0)
    
    n_ad_temp100 = n_ad_vec[index_temp][index_temp20]  #Note here n_ad_temp100 is the same as the n_ad_temp in the original prediction code 
    # update subcat_id
    database1[, 15:22] = rep(datam$subcat_id3[index_temp[index_temp20]], times = n_ad_temp100)
    
    # update npv, click, buy
    index_temp3 = rep(index_temp[index_temp20], times = n_ad_temp100)
    ...
    ctr_temp = ctrvec[temp2>0]
    
    ctr_temp = ctr_temp + 0.00001
    
    prob2 = (exp(S_temp[n_ad_temp>0]*ctr_temp))/sum( exp(S_temp[n_ad_temp>0]*ctr_temp ) )
    
    toadd = rmultinom(1, (8 -sum(n_ad_temp2[n_ad_temp>0])) , prob2)
    
    n_ad_temp2[n_ad_temp>0] = n_ad_temp2[n_ad_temp>0] + toadd
    
    
  }
    
    
    n_ad_vec[k:(k+n_pot-1)]=n_ad_temp2
...
while(m<=ndraw){
  ...
  while(i<=N_pv){ #i<=N_pv
    ...
    # change clicknew1-clicknew7 to 0
    database1[, 6:13] = 0
    # database1[, 6:13] = database0[, 6:13]
    # change rprice to 0
    database1[, 103:110] = 0
    # database1[, 103:110] = database0[, 103:110]
    ...
    database1$N_pot_subcat_full = database1$N_pot_subcat_full/50
    database1$Z_pvperday = database1$Z_pvperday/5
    ...
    pred_click = choiceprobpred(database1)  #here the choiceprobpred is the same as before
    # pred_click
    ctrvec = pred_click[1, ]
  ...
  if(m %% 1==0){
    ...
    output_14day=list(profit=profitmat, pay=paymat, cpc=cpcmat, click=clickmat, vpc=vpcmat, n_subcat_disp=n_subcat_disp_mat, avg_n_ad_disp=avg_n_ad_disp_mat)
    save(output_14day,
         file = "results_from_CF2.RData")
